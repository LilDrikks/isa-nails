...
function inputsData(e) {
  ...
  const mensage = `Ola Isa gostaria de saber se ha disponibilidade para o dia ${
    data[0] + "/" + data[1]
  } as ${inputTime}`;
  const msgWhatsSplit = mensage.split(" ");
  const msgWhats = msgWhatsSplit.join("%20");
  window.location.href = `https://wa.me/5511987033065?text=${msgWhats}`;
}
